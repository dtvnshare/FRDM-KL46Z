...
#ifdef BOARD_SPI_INSTANCE
  #define CDET_PIN spiCdPin
#else
...
{ /* Body */
...
#ifdef BOARD_SPI_GPIO_INSTANCE
   configure_gpio_pins(GPIO_EXTRACT_PORT(BOARD_SPI_GPIO_INSTANCE));
#endif
   ...
   //_nio_dev_install("spi:", &nio_esdhc_dev_fn, (void*)&_bsp_esdhc_init, NULL);

   /* get an instance of the SDHC driver */
   spi_handle = open("spi:", 0);
   ...
   sdcard_init_data.com_dev = spi_handle;
   sdcard_init_data.const_data = (SDCARD_CONST_INIT_STRUCT_PTR)&sdcard_spi_init_data;
